...
class Server(Thread):
    ...
    def run(self):
        ...
        while self.running:
            ...
            # error_list = []
            try:
                read_list, self.listen_sockets, error_list = select.select(self.clients, self.clients, [], wait)
            except OSError as e:
                # logger.error(f"Ошибка работы с сокетом: {e.errno}")
                pass
            ...
            if read_list:
                for client in read_list:
                    try:
                        ...
                        logger.debug(f"Возникло исключение клиента.", exc_info=e)
    ...
    def process_client_message(self, message, client):
        ...
        elif (message[ACTION] == MESSAGE and RECIPIENT in message and TIME in message
        ...
        elif message[ACTION] == GET_CONTACTS and self.names[message[USER]] == client:
        ...
        elif message[ACTION] == ADD_CONTACT and self.names[message[USER]] == client:
        ...
        elif message[ACTION] == DEL_CONTACT and self.names[message[USER]] == client:
        ...
        elif message[ACTION] == USERS_REQUEST and self.names[message[ACCOUNT_NAME]] == client:
            response = RESPONSE_202
            response[DATA_AVAILABLE] = [user[0] for user in self.db.users_list()]
            try:
                send_message(client, response)
            except OSError:
                self.remove_client(client)
    ...
    def auth_user(self, message, client):
        ...
        if message[USER][ACCOUNT_NAME] in self.names.keys():
            ...
            try:
                logger.debug(f"Неизвестный пользователь. Отправлено: {response}")
                send_message(client, response)
            except OSError:
                pass
            self.clients.remove(client)
            ...
            hash_string = hmac.new(self.db.get_hash(message[USER][ACCOUNT_NAME]), random_str, 'MD5')
            ...
            try:
                ...
                logger.debug("Ошибка в аутентификационных данных:", exc_info=e)
                client.close()
                return
    ...
    def process_message(self, message):
        ...
        if message[RECIPIENT] in self.names and self.names[message[RECIPIENT]] in self.listen_sockets:
            try:
                ...
                logger.info(f"Пользователю {message[RECIPIENT]} "
                            f"отправлено сообщение от пользователя {message[SENDER]}.")
            except OSError:
            ...
            logger.error(f"Связь с клиентом {message[RECIPIENT]} потеряна. Соединение закрыто.")
